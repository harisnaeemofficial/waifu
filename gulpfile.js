...
gulp.task('install:ffmpegsumo', function() {
    ...
    // Update path for OSX
    if (platform == 'osx')
        destPath += 'Contents/Frameworks/node-webkit Framework.framework/Libraries';

    // Copy lib
    return gulp.src(
        './deps/ffmpegsumo/'+platform+'/*'
    ).pipe(
        gulp.dest(destPath)
    )
})
    ...
    nw.build(function(err) {
        ...
        if (platforms.indexOf('linux32') > -1) {
            if (process.platform.indexOf('linux') > -1) {
                ...
                .pipe(
                    exec("sed -i 's/udev\.so\.0/udev.so.1/g' '<%= file.path %>'")
                );
            }
        }
